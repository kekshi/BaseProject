...
public class CrystalRangeSeekbar extends View {
    ...
    private float thumbWidth; //传入图片的宽，未传默认为24dp
    private float thumbHeight; //传入图片的高，未传默认为24dp
    private float thumbDiameter;
    private Bitmap leftThumb; //外部传入的左边图片资源
    private Bitmap leftThumbPressed; //外部传入的左边按下图片资源
    ...
    public CrystalRangeSeekbar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            leftThumb = getBitmap(getLeftDrawable(array));
            rightThumb = getBitmap(getRightDrawable(array));
            leftThumbPressed = getBitmap(getLeftDrawablePressed(array));
            rightThumbPressed = getBitmap(getRightDrawablePressed(array));
...
//        barHeight = getBarHeight();
    ...
    protected float getThumbDiameter() {
        return (thumbDiameter > 0) ? thumbDiameter : DensityUtils.INSTANCE.dp2px(24f);
    }
    ...
    protected float getDiameter(final TypedArray typedArray) {
        return typedArray.getDimensionPixelSize(R.styleable.CrystalRangeSeekbar_thumb_diameter, DensityUtils.INSTANCE.dp2px(24f));
    }
