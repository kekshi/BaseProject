...
public class SwipeCaptcha extends AppCompatImageView {
    private final String TAG = "SwipeCaptcha";
    //控件的宽高
    ...
    public SwipeCaptcha(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SwipeCaptcha(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray ta = context.getTheme().obtainStyledAttributes(attrs, R.styleable.SwipeCaptcha, defStyleAttr, 0);
        int n = ta.getIndexCount();
        for (int i = 0; i < n; i++) {
            int attr = ta.getIndex(i);
            if (attr == R.styleable.SwipeCaptcha_captchaHeight) {
                mCaptchaHeight = (int) ta.getDimension(attr, defaultSize);
            } else if (attr == R.styleable.SwipeCaptcha_captchaWidth) {
                mCaptchaWidth = (int) ta.getDimension(attr, defaultSize);
            } else if (attr == R.styleable.SwipeCaptcha_matchDeviation) {
    ...
    private void createMatchAnim() {
        ...
        mFailAnim.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                onCaptchaMatchCallback.matchFailed(SwipeCaptcha.this);
            }
        });
        ...
        mSuccessAnim.addListener(new AnimatorListenerAdapter() {
            ...
            @Override
            public void onAnimationEnd(Animator animation) {
                onCaptchaMatchCallback.matchSuccess(SwipeCaptcha.this);
    ...
    public interface OnCaptchaMatchCallback {
        void matchSuccess(SwipeCaptcha rxSwipeCaptcha);

        void matchFailed(SwipeCaptcha rxSwipeCaptcha);
    }
     ...
     * @return
     */
    public SwipeCaptcha setOnCaptchaMatchCallback(OnCaptchaMatchCallback onCaptchaMatchCallback) {
