...
//        heights = new int[2];
    ...
    public void isEnabledAnimator(boolean isAnabled) {
...
//            return;
        ...
        if (line <= mCollapsedLines) {
            ...
            if (lastLineWidth + expandedTextWidth > mShowWidth) {
                ...
                Log.e("ddddd", "cutCount is :" + cutCount);
                lastLineEnd -= cutCount;
            }
            // 因设置的文本可能是带有样式的文本，如SpannableStringBuilder，所以根据计算的字符数从原始文本中截取
            SpannableStringBuilder spannable = new SpannableStringBuilder();
            // 截取文本，还是因为原始文本的样式原因不能直接使用paragraphs中的文本 ,-2是给图片留位置
            CharSequence ellipsizeText = mOriginalText.subSequence(0, lastLineEnd - 2);
    ...
    private void startAnimat() {
        if (mIsExpanded) {
            objectAnimator = ObjectAnimator.ofInt(this, "height", heights[0]);
        } else {
            objectAnimator = ObjectAnimator.ofInt(this, "height", heights[1]);
        }
